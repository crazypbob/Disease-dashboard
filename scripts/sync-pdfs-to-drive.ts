 ...
 *
 * 사용법:
 *   npx tsx scripts/sync-pdfs-to-drive.ts --base="X:/.../검사결과_PDF"
 *   npx tsx scripts/sync-pdfs-to-drive.ts --base=... --dry-run
 *   npx tsx scripts/sync-pdfs-to-drive.ts --base=... --since=2026-04
 *   npx tsx scripts/sync-pdfs-to-drive.ts --base=... --rel=2026-03/a.pdf
 *   npx tsx scripts/sync-pdfs-to-drive.ts --base=... --rels-file=scripts/.drive_sync_rels.txt
 *
...
function parseArgs() {
  ...
  return { base, dryRun, sleepMs: Number.isFinite(sleepMs) && sleepMs > 0 ? sleepMs : 0, since, rels, relsFile };
}
...
async function main() {
  const { base, dryRun, sleepMs, since, rels, relsFile } = parseArgs();
  if (!base) {
  ...
  console.log(`Drive 동기화 · base=${baseAbs} · 대상 ${relList.length}개${dryRun ? ' (--dry-run)' : ''}`);
